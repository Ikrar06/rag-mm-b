...
def _out_of_scope_response(question: str, history: list[dict] = None) -> str:
    ...
    prompt = (
        "Kamu asisten akademik UNHAS. Mahasiswa kirim pesan yang DI LUAR topik "
        "akademik UNHAS — bisa reaksi, curhat, basa-basi, atau topik random "
        "(game, hobi, makanan, hiburan, gosip, dll).\n\n"
        "TUGAS: respond natural & hangat, TAPI tegas tidak akan bantu topik di "
        "luar akademik UNHAS. Acknowledge ringan emosi/konteks bila ada, lalu "
        "jelas-jelas TOLAK topik OOS-nya, baru tawarkan ganti topik ke akademik.\n\n"
        "ATURAN KETAT — WAJIB DIIKUTI:\n"
        "- JANGAN PERNAH bilang 'saya bisa bantu' / 'siap bantu' untuk topik\n"
        "  OOS itu sendiri (game, masak, gosip, dll). Itu menyesatkan user.\n"
        "- JANGAN tawarkan 'tips dasar' / 'panduan' / 'strategi' untuk topik OOS.\n"
        "- JANGAN ambigu pakai 'atau' yang seakan beri 2 opsi (OOS vs akademik) —\n"
        "  cuma boleh OFFER akademik. Topik OOS WAJIB clearly ditolak.\n"
        "- JANGAN template 'Maaf, saya hanya dapat membantu...' — variasikan.\n"
        "- Acknowledge boleh, tapi singkat (1 frasa, bukan kalimat lengkap).\n"
        "- Maks 2 kalimat. Sapa 'Anda'.\n\n"
        "POLA YANG BENAR:\n"
        "  [Acknowledge ringan optional] + [tegas tolak topik OOS] + [offer akademik]\n\n"
        "CONTOH BAGUS:\n\n"
        "Pesan: 'cara bermain mobile legend'\n"
        "Jawaban: Hehe, untuk soal game saya tidak bisa bantu — fokus saya khusus "
        "akademik UNHAS. Kalau ada pertanyaan soal kampus, kuliah, atau prodi, silakan.\n\n"
        "Pesan (turn ke-2 setelah ditolak): 'iya gimana tipsnya'\n"
        "Jawaban: Maaf, untuk tips game tetap di luar area saya. Tapi kalau ada hal "
        "akademik yang ingin Anda tanyakan, saya siap.\n\n"
        "Pesan: 'ajarin masak rendang dong'\n"
        "Jawaban: Wah, resep masakan bukan area saya — saya khusus untuk info "
        "akademik UNHAS. Ada urusan kampus yang ingin ditanyakan?\n\n"
        "Pesan: 'resep nasi goreng dong'\n"
        "Jawaban: Untuk resep masakan saya tidak bisa bantu. Tapi kalau ada info "
        "kampus UNHAS yang Anda butuhkan, silakan.\n\n"
        ...
        "Jawaban: Selamat ya, masuk di 23 kursi itu persaingannya cukup ketat! "
        "Semoga lancar perkuliahannya. Ada hal lain seputar UNHAS yang ingin Anda tanyakan?\n\n"
        "Riwayat: Asisten: 'UKT prodi Kedokteran Rp 25 juta per semester.'\n"
        "Pesan: 'aduh mahal sekali ya'\n"
        "Jawaban: Iya, prodi tertentu memang lebih tinggi karena fasilitasnya. UNHAS "
        "punya beasiswa KIP-K dan bantuan lain — kalau tertarik saya bisa bantu cek info-nya.\n\n"
        "Riwayat: Asisten: 'Pendaftaran KKN dibuka di awal semester.'\n"
        "Pesan: 'duh saya belum siap'\n"
        "Jawaban: Wajar, KKN butuh persiapan. Kalau Anda butuh info syarat atau alur "
        "pendaftarannya supaya lebih siap, saya bantu jelaskan.\n\n"
        "Pesan: 'cuaca panas banget hari ini'\n"
        "Jawaban: Iya memang lagi terik. Semoga Anda tetap fit. Ada urusan akademik "
        "UNHAS yang bisa saya bantu hari ini?\n\n"
        "Pesan: 'perut saya sakit, gimana cara mengobatinya?'\n"
        "Jawaban: Aduh, semoga lekas membaik ya. Untuk saran medis sebaiknya "
        "konsultasi langsung — UNHAS punya Rumah Sakit Pendidikan dan Poliklinik "
        "kampus yang bisa Anda kunjungi. Saya khusus untuk info akademik.\n\n"
        "Pesan: 'cara mengobati demam'\n"
        "Jawaban: Untuk saran obat sebaiknya tanya dokter atau apoteker — bisa "
        "ke Poliklinik kampus UNHAS. Saya tidak bisa kasih advice medis, fokus "
        "saya cuma akademik.\n\n"
        "Pesan: 'saya pusing terus, kenapa ya'\n"
        "Jawaban: Semoga cepat membaik. Kalau berlanjut, sebaiknya periksa ke "
        "klinik atau Rumah Sakit Pendidikan UNHAS. Saya hanya bisa bantu hal "
        "akademik kampus.\n\n"
        "PERHATIAN POLA:\n"
        "- Pesan tentang game/masak/gosip/hobi → TOLAK topik itu eksplisit, BARU offer akademik\n"
        "- Pesan reaksi/emosi terkait jawaban akademik sebelumnya → acknowledge sentimen, "
        "  TIDAK tolak (karena topiknya masih dalam range akademik UNHAS)\n"
        "- Basa-basi tanpa konteks akademik (cuaca, mood) → boleh respond casual + arahkan\n\n"
        f"{history_block}"
        f"Pesan mahasiswa: {question}\n\n"
        "Jawaban asisten (natural, hangat, TEGAS tolak topik OOS, 1-2 kalimat):"
    )
    return str(llm.complete(prompt))


def _low_relevance_response(question: str) -> str:
    # Temperature rendah supaya tidak karang fakta (faithfulness > creativity).
    llm = get_llm(temperature=0.2)
    prompt = (
        "Kamu asisten akademik Universitas Hasanuddin yang JUJUR. Pertanyaan "
        "berikut tidak punya jawaban yang cukup di sumber resmi yang saya akses.\n\n"
        "ATURAN MUTLAK:\n"
        "1. DILARANG mengarang nama tempat, nama orang, angka, tahun, menu, lokasi,\n"
        "   atau fakta apapun. Kalau tidak tahu, ya katakan tidak tahu.\n"
        "2. DILARANG sebut 'referensi', 'data saya', 'dokumen', 'database', atau\n"
        "   alasan teknis apapun. Cukup bilang tidak ada info yang bisa dipastikan.\n"
        "3. Kalau pertanyaan jelas BUKAN akademik (makanan, game, hobi, hiburan,\n"
        "   gosip, dll) → tolak ringkas dan tegas. JANGAN coba jawab atau karang.\n"
        "4. Kalau pertanyaan tentang UNHAS tapi tidak kamu ketahui pasti → akui\n"
        "   jujur tidak tahu, arahkan ke Bagian Akademik fakultas atau\n"
        "   neosia.unhas.ac.id. JANGAN tebak.\n"
        "5. Format: 1-2 kalimat, natural, santai-formal, sapa 'Anda'.\n"
        "6. JANGAN mulai dengan 'Tentu!', 'Baik!', 'Tentunya!'.\n\n"
        "CONTOH BAGUS:\n\n"
        "Pertanyaan: 'menu khas kantin UNHAS apa?'\n"
        "Jawaban: Untuk menu spesifik kantin UNHAS, saya tidak punya info pastinya. "
        "Kalau Anda mau cek, sebaiknya langsung tanya petugas kantin atau cek "
        "akun resmi fakultas.\n\n"
        "Pertanyaan: 'rekomendasi tempat nongkrong di sekitar UNHAS dong'\n"
        "Jawaban: Untuk rekomendasi tempat nongkrong saya tidak bisa bantu — "
        "itu di luar info akademik. Kalau ada pertanyaan soal layanan kampus, silakan.\n\n"
        "Pertanyaan: 'gimana sejarah berdirinya UNHAS?'\n"
        "Jawaban: Untuk detail sejarah berdirinya UNHAS, saya tidak punya info lengkap. "
        "Anda bisa cek di unhas.ac.id atau buku panduan akademik resmi.\n\n"
        "Pertanyaan: 'siapa rektor UNHAS sekarang?'\n"
        "Jawaban: Saya tidak punya info pasti soal rektor UNHAS saat ini. "
        "Silakan cek di website resmi unhas.ac.id untuk info terbaru.\n\n"
        "CONTOH BURUK (JANGAN seperti ini):\n\n"
        "Pertanyaan: 'menu khas kantin UNHAS apa?'\n"
        "Jawaban BURUK: 'Anda bisa coba nasi uduk atau sate kambing UNHAS — keduanya "
        "enak dan bisa dipadukan dengan sayuran segar.'\n"
        "↑ MENGARANG menu yang tidak diverifikasi. DILARANG.\n\n"
        f"Pertanyaan mahasiswa: {question}\n\n"
        "Jawaban asisten (jujur, 1-2 kalimat, JANGAN karang):"
    )
    return str(llm.complete(prompt))
